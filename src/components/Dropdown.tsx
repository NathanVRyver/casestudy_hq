...
>(({ className, children, ...props }, forwardedRef) => (
  <DropdownMenuPrimitives.SubTrigger
    ...
    className={cx(
      ...
      // focus
      "focus-visible:bg-stone-100 data-[state=open]:bg-stone-100 focus-visible:dark:bg-stone-800 data-[state=open]:dark:bg-stone-800",
      // hover
      "hover:bg-stone-100 hover:dark:bg-stone-800",
      //
      className,
    )}
    {...props}
...
>(({ className, shortcut, hint, children, ...props }, forwardedRef) => (
  <DropdownMenuPrimitives.Item
    ...
    className={cx(
      ...
      // focus
      "focus-visible:bg-stone-100 focus-visible:dark:bg-stone-800",
      // hover
      "hover:bg-stone-100 hover:dark:bg-stone-800",
      className,
    )}
  ...
  >
    ...
    {hint && (
      <span
        className={cx("ml-auto pl-2 text-sm text-stone-400 dark:text-stone-500")}
      >
        {hint}
      </span>
    )}
    ...
    >
      <span className="absolute left-2 flex size-4 items-center justify-center">
        <DropdownMenuPrimitives.ItemIndicator>
          <RiCheckLine
            aria-hidden="true"
            className="size-full shrink-0 text-stone-800 dark:text-stone-200"
          />
        </DropdownMenuPrimitives.ItemIndicator>
      </span>
      {children}
      {hint && (
        <span
          className={cx(
            "ml-auto text-sm font-normal text-stone-400 dark:text-stone-500",
          )}
        >
          {hint}
        </span>
    ...
    >
      {iconType === "radio" ? (
        <span className="absolute left-2 flex size-4 items-center justify-center">
          <RiRadioButtonFill
            aria-hidden="true"
            className="size-full shrink-0 text-stone-900 dark:text-stone-50 group-data-[state=checked]/DropdownMenuRadioItem:flex group-data-[state=unchecked]/DropdownMenuRadioItem:hidden dark:text-stone-900 dark:text-stone-50"
          />
          <RiCheckboxBlankCircleLine
            aria-hidden="true"
            className="size-full shrink-0 text-stone-300 group-data-[state=unchecked]/DropdownMenuRadioItem:flex group-data-[state=checked]/DropdownMenuRadioItem:hidden dark:text-stone-600"
          />
        </span>
      ) : iconType === "check" ? (
        <span className="absolute left-2 flex size-4 items-center justify-center">
...
>(({ className, ...props }, forwardedRef) => (
  <DropdownMenuPrimitives.Separator
    ref={forwardedRef}
    className={cx(
      "-mx-1 my-1 h-px border-t border-stone-200 dark:border-stone-800",
      className,
    )}
    {...props}
  />
  ...
  return (
    <div
      className={cx(
        ...
        // disabled
        "group-data-[disabled]/DropdownMenuItem:text-gray-400 group-data-[disabled]/DropdownMenuItem:dark:text-stone-600",
        className,
      )}
      {...props}
    />
